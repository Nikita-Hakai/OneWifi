...
/* unsigned char binaryData[] = {
    0x27, 0x26, 0x01, 0x00, 0x05, 0x01, 0x24, 0x00,
    0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x2E, 0xA9,
    0xBE, 0xB3, 0x13, 0x03, 0x66, 0xB5, 0x00, 0x00,
    0x00, 0x00, 0x01, 0x19, 0x00, 0x00, 0x00, 0x00,
    0x00, 0x00, 0x00, 0x0B, 0x78, 0x66, 0x69, 0x6F
}; */

unsigned char binaryData[] = {0x27, 0x39, 0x1, 0x0, 0x5, 0x1, 0x24, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x58, 0xbe, 0xd4, 0x52, 0xee, 0xe, 0x11, 0xcb, 0x0, 0x0, 0x0, 0x0, 0x0, 0x1, 0x16, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x0, 0x8, 0x53, 0x4b, 0x59, 0x37, 0x39, 0x42, 0x35, 0x37, 0x2, 0x2, 0x1, 0x80};
static int send_em_test_data(void *arg)
{
    ...
    temp_data_t.num_br_data = 1;
    temp_data_t.ap_index = 1;

    //mac_addr_t sta_mac = {0x82,0x5c,0xec,0x20,0xc0,0xd9};
    //52:09:79:50:9f:dd
    mac_addr_t sta_mac = {0x52,0x09,0x79,0x50,0x9f,0xdd};
...
#if TEST_CODE
        scheduler_cancel_timer_task(ctrl->sched, &(g_sched_id));
#endif
        free(t_sta_data);
    }
    return 0;
...
static int sta_mgr_handle_assoc_device(wifi_app_t *app, void *arg)
{
    ...
    if ((ctrl->network_mode == rdk_dev_mode_type_gw ||
            ctrl->network_mode == rdk_dev_mode_type_em_colocated_node ||
            ctrl->network_mode == rdk_dev_mode_type_em_node)) {
        sta_beacon_report_reponse_t *t_sta_data = hash_map_get(app->data.u.sta_mgr.sta_mgr_map,
            client_mac);
        if (t_sta_data != NULL) {
            ...
            wifi_util_dbg_print(WIFI_APPS, "%s:%d : ap_index %d \n", __func__, __LINE__,
                t_sta_data->ap_index);
...
#define OUI_SKY 0x807215
#define ASSOC_REQ_MAC_HEADER_LEN \
    24 + 2 + 2 // 4 bytes after mac header reserved for fixed len fields

char *hexBytesToString(const uint8_t *hexBytes, size_t len, char *output)
{
    for (size_t i = 0; i < len; i++) {
        output[i] = (char)hexBytes[i]; // Directly convert each byte to a character
    }
...
static bool sta_mgr_parse_vendor_specific_info(wifi_app_t *app, const uint8_t *pos, size_t elen,
...
static bool is_marker_present(wifi_app_t *app, const uint8_t *ie, size_t ie_len)
{
    const struct element *elem;
    for_each_element_id(elem, WLAN_EID_VENDOR_SPECIFIC, ie, ie_len)
    {
        size_t elen = elem->datalen;
...
void sta_mgr_handle_assoc_req(wifi_app_t *app, void *arg)
{
    frame_data_t *mgmt_frame = (frame_data_t *)arg;
    const struct ieee80211_mgmt *mgmt = (struct ieee80211_mgmt *)mgmt_frame->data;
    size_t len = mgmt_frame->frame.len;
...
int sta_mgr_hal_event_sta_mgr(wifi_app_t *apps, wifi_event_subtype_t sub_type, void *arg)
{
    ...
    case wifi_event_hal_assoc_req_frame:
        wifi_util_info_print(WIFI_APPS, "%s:%d: wifi_event_hal_assoc_req_frame \n", __func__,
            __LINE__);
        sta_mgr_handle_assoc_req(apps, arg);
        break;
    default:
...
void process_dhcp_packet(wifi_app_t *app, const uint8_t *buffer, ssize_t len)
{
    sta_client_info_t sta_info;
    struct iphdr *ip_header;
    struct udphdr *udp_header;
    struct dhcp_data *dhcp;
    uint8_t *ptr;
    uint8_t *options;
    unsigned char hostname[50], vendor_class_id[50];
    int found = 0;
    int dhcp_hdr_sz = sizeof(struct ethhdr) + sizeof(struct iphdr) + sizeof(struct udphdr);
    int dhcp_data_offset = dhcp_hdr_sz + sizeof(struct dhcp_data) + 4;
    ssize_t options_len = len - (dhcp_data_offset + 1); // includes End Byte

    dhcp = (struct dhcp_data *)(buffer + dhcp_hdr_sz);
    ptr = (uint8_t *)dhcp;
    options = (uint8_t *)(buffer + dhcp_data_offset);
    uint8_t option_type = options[0];
    uint8_t current_op_len = options[1];

    if ((dhcp->op == DHCP_BOOTP)) {
        if ((options[2] == DHCP_OP_DISCOVER_INDEX) || (options[2] == DHCP_OP_REQUEST_INDEX)) {
            memset(&sta_info, 0, sizeof(sta_info));
            while (options_len > 0) {
                option_type = options[0];
                current_op_len = options[1];

                if (option_type == DHCP_OPTION_HOSTNAME) {
                    memcpy(hostname, &options[2], current_op_len);
                    memcpy(sta_info.mac_addr, &ptr[28], sizeof(sta_info.mac_addr));
                    found = 1;
                    break;
                } else if (option_type == DHCP_OPTION_VENDOR_CLASS_ID) {
                    memcpy(vendor_class_id, &options[2], current_op_len);
                }

                options += current_op_len + 2;
                options_len -= current_op_len + 2;
            }
        }
    }
    ...
    if (found == 1) {
        ...
        if (vendor_class_id != NULL) {
            sscanf(vendor_class_id, "%[^- ]", first_word);
        }

        if (strlen(first_word) > 0) {
            sprintf(sta_info.client_type, "%s - %s", first_word, hostname);
        } else {
            sprintf(sta_info.client_type, "%s", hostname);
        }

        push_event_to_ctrl_queue((const void *)&sta_info, sizeof(sta_client_info_t),
            wifi_event_type_command, wifi_event_type_sta_client_info, NULL);
    }
}

void *dhcp_sniff_event_func(void *arg)
{
    ...
    if (raw_socket < 0) {
        wifi_util_dbg_print(WIFI_APPS, "%s:%d: Socket creation failed\n", __func__, __LINE__);
        return 1;
    }

    strncpy(ifr.ifr_name, "brlan0", IFNAMSIZ - 1);
    if (ioctl(raw_socket, SIOCGIFINDEX, &ifr) < 0) {
        wifi_util_info_print(WIFI_APPS, "%s:%d: Getting interface index failed\n", __func__,
            __LINE__);
        close(raw_socket);
        return 1;
    }
    ...
    if (bind(raw_socket, (struct sockaddr *)&sll, sizeof(sll)) < 0) {
        wifi_util_info_print(WIFI_APPS, "%s:%d: Binding socket to interface failed\n", __func__,
            __LINE__);
        close(raw_socket);
        return 1;
    }
    ...
    while (1) {
        ...
        if (packet_len < 0) {
            wifi_util_info_print(WIFI_APPS, "%s:%d: Packet reception failed\n", __func__, __LINE__);
            close(raw_socket);
            return 1;
        }

        // Process the packet
        process_dhcp_packet(app, buffer, packet_len);
    }
...
int sta_mgr_init(wifi_app_t *app, unsigned int create_flag)
{
    ...
    ret = pthread_create(&tid, &attr, dhcp_sniff_event_func, (void *)app);
    if (ret != 0) {
        wifi_util_info_print(WIFI_APPS,
            "%s:%d failed to create worker thread, "
            "err %d (%s)\n",
